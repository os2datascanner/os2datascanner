...
class Command(BaseCommand):
    ...
    def handle(self, account, scanner, *args, **options):
        ...
        if not account_obj:
            raise CommandError(f"Account with username “{account}” does not exist.")
        elif not scanner_obj:
            raise CommandError(f"Scanner with primary key “{scanner}” does not exist.")
        elif scanner_obj.statuses.last() and not scanner_obj.statuses.last().finished:
            raise CommandError(f"Scanner “{scanner_obj.name}” is currently running. Doing nothing.")
